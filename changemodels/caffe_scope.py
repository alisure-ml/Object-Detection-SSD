...
The idea is to create special initializers loading weights from protobuf .caffemodel files.
"""
...
class CaffeScope(object):
    ...
    def load(self, filename, bgr_to_rgb=True):
        ...
        # Layers collection.
        self.layers['convolution'] = [i for i, l in enumerate(self.caffe_layers) if l.type == 'Convolution']
        self.layers['l2_normalization'] = [i for i, l in enumerate(self.caffe_layers) if l.type == 'Normalize']
        # BGR to RGB convertion. Tries to find the first convolution with 3 and exchange parameters.
        self.bgr_to_rgb = 1 if bgr_to_rgb else self.bgr_to_rgb
